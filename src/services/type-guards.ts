...
export function isApiResponse<T>(
  ...
  if (!('message' in response) || !isStringOrNull(response.message))
    return false
  ...
  if ('meta' in response && response.meta !== null && !isObject(response.meta))
    return false
  if (
    'errors' in response &&
...
export function isRelicEffect(value: unknown): value is any {
  ...
  return (
    ...
    ('conditions' in effect
      ? isObject(effect.conditions) || effect.conditions === null
      : true)
  )
}
...
export function isRelic(value: unknown): value is Relic {
  ...
  if ('attackMultiplier' in relic && !isNumberOrNull(relic.attackMultiplier))
    return false
  if (
    'obtainmentDifficulty' in relic &&
    !isNumberOrNull(relic.obtainmentDifficulty)
  )
    return false
...
export function isBuild(value: unknown): value is Build {
  ...
  if ('attackMultiplier' in build && !isNumberOrNull(build.attackMultiplier))
    return false
  if (
    'obtainmentDifficulty' in build &&
    !isNumberOrNull(build.obtainmentDifficulty)
  )
    return false
...
export function isAttackMultipliers(value: unknown): value is any {
  ...
  return (
    'total' in multipliers &&
    isNumber(multipliers.total) &&
    'base' in multipliers &&
    isNumber(multipliers.base) &&
    'synergy' in multipliers &&
    isNumber(multipliers.synergy) &&
    'conditional' in multipliers &&
    isNumber(multipliers.conditional)
  )
}
...
export function isCalculationResult(
  ...
  // Check required fields
  if (
    !('attackMultipliers' in result) ||
    !isAttackMultipliers(result.attackMultipliers)
  )
    return false
  if (!('efficiency' in result) || !isNumber(result.efficiency)) return false
  if (
    !('obtainmentDifficulty' in result) ||
    !isNumber(result.obtainmentDifficulty)
  )
    return false

  // Check optional arrays
  if ('relicDetails' in result && !isArray(result.relicDetails)) return false
  if ('effectBreakdown' in result && !isArray(result.effectBreakdown))
    return false
  if ('calculationSteps' in result && !isArray(result.calculationSteps))
    return false
...
export function isOptimizationSuggestion(value: unknown): value is any {
  ...
  return (
    'id' in suggestion &&
    isString(suggestion.id) &&
    'type' in suggestion &&
    isString(suggestion.type) &&
    'priority' in suggestion &&
    isString(suggestion.priority) &&
    'title' in suggestion &&
    isString(suggestion.title) &&
    'description' in suggestion &&
    isString(suggestion.description) &&
    'expectedImprovement' in suggestion &&
    isNumber(suggestion.expectedImprovement)
  )
}
...
export function validatePaginatedResponse<T>(
  ...
  if (!paginatedResponse.success) {
    throw new Error(
      `${context} failed: ${paginatedResponse.message || 'Unknown error'}`
    )
  }
