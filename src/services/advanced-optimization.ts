...
class AdvancedOptimizationService {
  private combatStyleProfiles: Record<CombatStyle, CombatStyleProfile> = {
    melee: {
      name: 'melee',
      description:
        '近接戦闘に特化したスタイル。高い攻撃力と安定したダメージ出力を重視',
      ...
      strategies: [
        {
          ...
          conditions: [{ type: 'category_focus', value: 'Critical' }],
          relicWeights: {
            critical_chance: 1.4,
      ...
      strategies: [
        {
          ...
          conditions: [{ type: 'category_focus', value: 'Elemental' }],
          relicWeights: {
      ...
      strategies: [
        {
          ...
          conditions: [{ type: 'relic_count', value: { min: 7, max: 9 } }],
          relicWeights: {
  ...
  async optimizeForCombatStyle(
    ...
    // 現在のビルドを分析
    const currentAnalysis = await this.analyzeBuildPerformance(
      currentRelics,
      options.combatStyle
    )
    ...
    // 結果の検証と説明生成
    const optimizedAnalysis = await this.analyzeBuildPerformance(
      optimizedBuild.relics,
      options.combatStyle
    )
  ...
  async analyzeMetaBuilds(
    ...
    return analyses.sort(
      (a, b) => b.performance.attackMultiplier - a.performance.attackMultiplier
    )
  }
  ...
  async optimizeByDifficulty(
    ...
    const filteredRelics = availableRelics.filter(
      relic =>
        relic.obtainmentDifficulty >= range.min &&
        relic.obtainmentDifficulty <= range.max
    )
    ...
    const options: AdvancedOptimizationOptions = {
      ...
      prioritizeCategories:
        this.combatStyleProfiles[combatStyle].preferredCategories,
      minEfficiency:
    ...
    const result = await this.optimizeForCombatStyle(
      currentRelics,
      filteredRelics,
      options
    )
  ...
  private generateOptimizationExplanation(
    ...
    const improvement =
      optimizedAnalysis.attackMultipliers.total -
      currentAnalysis.attackMultipliers.total
    const improvementPercent =
      (improvement / currentAnalysis.attackMultipliers.total) * 100
    ...
    const tradeoffs = changes.map(change => ({
      ...
      pros: [`攻撃倍率 +${change.improvement.toFixed(2)}`, change.reason],
  ...
  private calculateRelicScore(
    ...
    const difficultyPenalty =
      options.difficultyPreference === 'easy'
        ? Math.max(0, (relic.obtainmentDifficulty - 5) * 0.1)
        : 0
  ...
  private async generateOptimizedBuild(
    ...
    for (let i = 0; i < Math.min(5, candidateRelics.length); i++) {
      ...
      if (workingRelics.length < options.maxRelics) {
        ...
        // 最も効果の低い遺物と置換を検討
        const worstRelicIndex = await this.findWorstRelic(
          workingRelics,
          options.combatStyle
        )
  ...
  private async findWorstRelic(
    ...
    for (let i = 0; i < relics.length; i++) {
      ...
      const testResult = await this.analyzeBuildPerformance(
        testRelics,
        combatStyle
      )
      const impact =
        baseResult.attackMultipliers.total - testResult.attackMultipliers.total
   ...
   */
  private async fetchMetaBuildsData(
    combatStyle: CombatStyle,
    constraints?: any
  ) {
    // 実際の実装では API から取得
   ...
   * メタビルドを分析
   */
  private async analyzeMetaBuild(
    build: any,
    combatStyle: CombatStyle
  ): Promise<MetaBuildAnalysis> {
