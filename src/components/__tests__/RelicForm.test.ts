...
describe('RelicForm', () => {
  ...
  describe('rendering', () => {
    ...
    it('should show form title based on mode', () => {
      const { wrapper: createWrapper, assert: createAssert } =
        createTestEnvironment(RelicForm, {
          props: { mode: 'create' },
        })
      createAssert.expectTextContent(
        '[data-testid="form-title"]',
        'Create New Relic'
      )

      const { wrapper: editWrapper, assert: editAssert } =
        createTestEnvironment(RelicForm, {
          props: { mode: 'edit' },
        })
  ...
  describe('form validation', () => {
    ...
    it('should show validation errors for effects', async () => {
      ...
      assert.expectTextContent(
        '[data-testid="effect-0-name-error"]',
  ...
  describe('effects management', () => {
    ...
    it('should populate effect fields', async () => {
      ...
      await user.typeInInput(
        '[data-testid="effect-0-description"]',
        'Increases attack damage'
      )
      await user.selectOption(
        '[data-testid="effect-0-type"]',
        'attack_multiplier'
      )
      await user.typeInInput('[data-testid="effect-0-value"]', '1.25')

      assert.expectFormFieldValue(
        '[data-testid="effect-0-name"]',
        'Attack Boost'
      )
      assert.expectFormFieldValue(
        '[data-testid="effect-0-description"]',
    ...
    it('should validate effect values based on type', async () => {
      ...
      assert.expectTextContent(
        '[data-testid="effect-0-value-error"]',
  ...
  describe('conditions management', () => {
    ...
    it('should validate condition values', async () => {
      ...
      assert.expectTextContent(
        '[data-testid="condition-0-value-error"]',
        'Health percentage must be between 1 and 100'
      )
    })
  })
  ...
  describe('accessibility', () => {
    it('should have proper form labels', () => {
      ...
      inputs.forEach(selector => {
        ...
        expect(
          input.attributes('aria-label') || input.attributes('id')
        ).toBeTruthy()
      })
    })
    ...
    it('should associate error messages with inputs', async () => {
      ...
      expect(nameInput.attributes('aria-describedby')).toContain(
        nameError.attributes('id')
      )
  ...
  describe('draft saving', () => {
    it('should auto-save draft periodically', async () => {
      ...
      const savedDraft = JSON.parse(
        localStorage.getItem('relicFormDraft') || '{}'
      )
