  ...
  describe('actions', () => {
    ...
    describe('selectRelic', () => {
      ...
      it('should not select more than maximum allowed relics', async () => {
        ...
        expect(store.selectedRelics.length).toBe(
          Math.min(9, store.relics.length)
        )
      ...
      it('should handle selecting already selected relic', async () => {
        ...
        if (relicId) {
          ...
          expect(store.selectedRelics.filter(id => id === relicId).length).toBe(
            1
          )
        }
      })
    ...
    describe('applyQuickFilter', () => {
      it('should apply category filter', async () => {
        ...
        expect(store.filteredRelics.every(r => r.category === 'Attack')).toBe(
          true
        )
      })
      ...
      it('should apply rarity filter', async () => {
        ...
        expect(store.filteredRelics.every(r => r.rarity === 'legendary')).toBe(
          true
        )
      })
      ...
      it('should combine multiple filters', async () => {
        ...
        expect(
          store.filteredRelics.every(
            r => r.category === 'Attack' && r.rarity === 'rare'
          )
        ).toBe(true)
      })
    })
